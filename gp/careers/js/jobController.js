myApp.controller('jobController', function($scope, $http, $stateParams, $sce, $ngConfirm) {
 ...
 console.log("Gautham Job Scope",jobID);
        $('html, body').animate({
    ...
    $scope.sendJob = function() {
       // console.log("form values:",$scope.applyForm);
        $http({
            method: "POST",
            data: {
    ...
    $scope.jobSubmit = function() {
     console.error("File: ",$scope.fileUpload);
        if ($scope.fileUpload==null) {
           // console.clear();
        ...
        } else if ($scope.applyForm.gender == null || $scope.applyForm.ethnicity == null || $scope.applyForm.veteran == null || $scope.applyForm.disability == null  ){
            $scope.gerror = 'All fields are mandatory, please check again';
        } else {
            ...
            if ($scope.fileUpload.filesize > 12*1024*1024) {                
               ...
               console.log("Filesize: ",$scope.fileUpload.filesize)
